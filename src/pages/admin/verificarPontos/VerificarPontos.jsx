...
import styles from "./VerificarPontos.module.css";
import { useLocation } from "react-router-dom";

export default function VerificarPontos() {
    ...
    const carregarSemana = async () => {
        ...
        const response = await axios.get(
            `http://localhost:8080/funcionario/buscarPontos/${idFuncionario}`,
    ...
    const buscarPorDia = async (data) => {
        ...
        try {
        const response = await axios.get(
            `http://localhost:8080/funcionario/buscarPonto/${idFuncionario}/${data}`,
            { withCredentials: true }
        );
    ...
    return (
        ...
        <h1 className={styles.tituloPrincipal}>Registro de pontos - <span className={styles.nomeFunc}>{nome}</span></h1>
